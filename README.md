...
- 🔒 **本地优先** —— 面板与记忆库(SQLite)都在 `127.0.0.1`,无云同步、无账号。但**提纯 / 嵌入 / 语义召回会把相应内容发给你配的云端 LLM**(默认阿里云百炼)——这部分会出本机;浏览 / 关键词检索 / 备份则全程离线。详见下方「数据去哪儿了」。
...
> 本仓库仅含**代码 / schema / 脚本**,**不含任何个人记忆数据**。你的记忆库只存在你自己机器上。

### 数据去哪儿了
面板服务只在本机,但需要"理解"文本的步骤会调用你所配的云端 LLM(默认阿里云百炼):
- **会出本机**:记忆提纯(整段对话原文)、向量嵌入(记忆结论 / 查询词)、语义召回(查询词)——都发给你配的 provider。
- **全程本地**:浏览 / 筛选 / 关键词检索(本地 SQLite)、备份、原始归档、Markdown 投影,以及可选的「检查更新」(只拉版本号)。
- 想完全离线可后续接本地嵌入模型(规划中)。填 key = 同意把上述内容发往该 provider。
